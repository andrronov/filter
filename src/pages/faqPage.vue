<template>
  <page-wrapper>
    ...
    >
      ...
      <template #r>
        <div
          class="w-full px-5 rounded-lg shadow-md md:self-start mt-64 md:mt-4"
        >
          ...
          <form @submit.prevent="handleSubmit">
            ...
            <!-- Options -->
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >Please select one of the options below<span class="text-red-600"
                >*</span
              ></label
            >
            <select
            ...
            <input
              ...
              required
            />
            <p class="text-[#929292] -mt-3 mb-2 text-sm xs:text-base">
            ...
            >
              <span v-if="!fileName" class="text-sm"
                >Add file or drop files here</span
              >
...
function search() {
  selectedItem.value =
    menuItems.value.filter((item) =>
      item.toLowerCase().includes(searchItem.value.toLowerCase())
    )[0] || "";
}
