    ...
    <page-section v-for="(item, index) in landingItems" :key="index">
      <template #l>
        <div class="flex flex-col items-start gap-2">
          ...
          <text-text
            v-if="item.text"
            :text="item.text"
            class="w-full max-w-[343px] xs:max-w-[366px]"
          />
          <download-buttons
            v-if="item.isDownloadButtons"
            :downloadble="index === landingItems.length - 1"
          />
        </div>
      </template>
      ...
      <template #r>
        <img
          :src="item.img"
          class="w-full h-full"
          :class="item.isPhotoCover ? 'object-cover' : 'object-contain'"
        />
      </template>
    </page-section>

    <section
      class="w-full h-screen min-h-screen flex flex-col justify-between relative z-50 snap-start bg-[#373737]"
    >
      <filter-text-icon :fill-black="false" class="w-full p-4 xs:p-5" />
      <footer
        class="w-full bg-[#373737] flex flex-row text-txt-100 items-end justify-between px-5 pb-5"
      >
...
const landingItems = shallowRef([
  ...
  },
  {
    ...
    text: "Explore new ones and subscribe to your favorite influencers, follow their selection and learn more about their taste",
    isDownloadButtons: false,
    img: "/filter/img/albums.png",
  },
  {
    isPhotoCover: false,
    title: "Get access to the taste of the influencer",
    isDownloadButtons: false,
    img: "/filter/img/socials.png",
  },
  {
    isPhotoCover: false,
